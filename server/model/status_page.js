...
class StatusPage extends BeanModel {
    ...
    static async getIncidentHistory(statusPageId, cursor = null, isPublic = true) {
        ...
        const incidentList = incidents.map((i) => i.toPublicJSON());
        // Soca: attach the lifecycle timeline to each incident in the history
        await StatusPage.attachIncidentUpdates(incidentList);
    ...
    /**
     * Soca: attach the lifecycle timeline (newest first) to each public incident object.
     * @param {object[]} incidents Array of public incident objects (must have an `id`)
     * @returns {Promise<object[]>} The same array, with `updates` populated on each item
     */
    static async attachIncidentUpdates(incidents) {
        for (const incident of incidents) {
            const updates = await R.find(
                "incident_update",
                " incident_id = ? ORDER BY created_date DESC, id DESC ",
                [incident.id]
            );
            incident.updates = updates.map((u) => ({
                status: u.status,
                message: u.message,
                createdDate: u.created_date,
            }));

            // Soca: attach the affected monitors (systems) related to this incident
            const monitors = await R.getAll(
                `SELECT im.monitor_id AS id, m.name AS name
                 FROM incident_monitor im
                 JOIN monitor m ON m.id = im.monitor_id
                 WHERE im.incident_id = ?`,
                [incident.id]
            );
            incident.affectedMonitors = monitors.map((m) => ({ id: m.id, name: m.name }));
        }
        return incidents;
    }
