...
class UptimeKumaServer {
    ...
    async sendUpdateMonitorIntoList(socket, monitorID) {
        ...
        if (list && list[monitorID]) {
            // Soca: monitors are shared team-wide — notify all dashboard clients.
            this.io.to(SHARED_DASHBOARD_ROOM).emit("updateMonitorIntoList", list);
        }
    }
    ...
    async sendDeleteMonitorFromList(socket, monitorID) {
        // Soca: monitors are shared team-wide — notify all dashboard clients.
        this.io.to(SHARED_DASHBOARD_ROOM).emit("deleteMonitorFromList", monitorID);
    }
    ...
    async getMonitorJSONList(userID, monitorID = null) {
        // Soca: monitors are shared across the whole team, not scoped per user.
        // `userID` is retained for signature compatibility but no longer filters.
        let query = " 1 = 1 ";
        let queryParams = [];
    ...
    async sendMaintenanceListByUserID(userID) {
        ...
        // Soca: maintenance windows are shared team-wide — notify all dashboard clients.
        this.io.to(SHARED_DASHBOARD_ROOM).emit("maintenanceList", list);
        return list;
    }
